...
const Step1 : React.FC<{
    nextStep : () => void,signUpData : any,setSignUpData : any
}> = ({nextStep,signUpData,setSignUpData}) => {
    const handleNextStep = () => {
        // 모든 필수 입력값을 배열로 관리
        const requiredFields = [signUpData.job || "", // 기본값을 빈 문자열로 설정
            signUpData.companyType || "",signUpData.representative || "",signUpData.location || "",];
        ...
        <div className="content">
            ...
            <div className="input_box">
                ...
                <select
                    ...
                    onChange={(e) => setSignUpData((signUpData : any) => ({
                        ...signUpData,companyType: e.target.value
                    }))}
                >
            ...
            <div className="input_box">
                ...
                <input
                    ...
                    onChange={(e) => setSignUpData((signUpData : any) => ({
                        ...signUpData,representative: e.target.value
                    }))}
                />
            </div>
            <div className="input_box">
                ...
                <input
                    ...
                    onChange={(e) => setSignUpData((signUpData : any) => ({
                        ...signUpData,location: e.target.value
                    }))}
                />
            </div>
        </div>
...
const Step2 : React.FC<{
    ...
    setSignUpData : any;
          ...
          nextStep,prevStep,handleEmailVerifyClick,isEmailVerified,signUpData,setSignUpData
      }) => {
    const [showVerification,setShowVerification] = useState(false); // 인증 입력 필드를 표시하기 위한 상태
    const [isDuplicate,setIsDuplicate] = useState(true);
    const [vrfnCode,setVrfnCode] = useState("");
    const vrfnId = useRef();

    const handleRequestVerification = () => {
        restApi('get','/admin/vrfn/mail/req',{
            mail: signUpData.email,
        }).then(response => {
            if (response.status === 200) {
                vrfnId.current = response.data.message;
    ...
    const handleNextStep = () => {
        const requiredFields = [signUpData.username || "",signUpData.password || "",signUpData.email || "",signUpData.businessType || "",signUpData.industry || "",];
    ...
    const handleCheckVerify = () => {
        restApi('post','/admin/vrfn/mail/check',{
            idx: vrfnId.current,verificationCode: vrfnCode,
        }).then(response => {
            if (response.status === 200) {
                alert(response.data.message);
    ...
    return (<div className="fade">
        <div className="content">
            ...
            <div className="input_box">
                ...
                {showVerification && !isEmailVerified && (<div>
                    <input
                        type="text"
                        placeholder="이메일 인증 키를 입력하세요"
                        value={vrfnCode}
                        onChange={(event) => {
                            setVrfnCode(event.target.value)
                        }}
                    />
                    <button type="button" onClick={(event) => {
                        handleCheckVerify()
            ...
            <div className="input_box">
                ...
                <input
                    ...
                    onChange={(e) => setSignUpData((signUpData : any) => ({
                        ...signUpData,businessType: e.target.value
                    }))}
                />
            </div>
            <div className="input_box">
                ...
                <input
                    ...
                    onChange={(e) => setSignUpData((signUpData : any) => ({
                        ...signUpData,industry: e.target.value
                    }))}
                />
            </div>
...
const Step3 : React.FC<{
    prevStep : () => void,submit : () => void,//  handleEmailVerifyClick: () => void,
    //   isEmailVerified: boolean,
    signUpData : any,setSignUpData : any
        ...
        <div className="content">
            <div className="input_box">
                ...
                <input
                    ...
                    onChange={(e) => setSignUpData((signUpData : any) => ({
                        ...signUpData,BusinessRegistrationNum: e.target.value
                    }))}
                />
            </div>
            <div className="input_box">
                ...
                <textarea
                    ...
                    onChange={(e) => setSignUpData((signUpData : any) => ({
                        ...signUpData,reason: e.target.value
                    }))}
                />
            </div>
            {/* <div className="input_box">
...
const Page : React.FC = () => {
    ...
    return (<>
        {/* <Modal /> */}
        {!showSignUp ? (<div className="admin_search_wrap">
            ...
            {outputMessage && (<div className="output">
                {outputMessage === "검색 내용이 없습니다" ? (<>
                    <p style={{color: "red"}}>{outputMessage}</p>
                    <button type="button" className="type1" onClick={() => {
                        setShowSignUp(true)
                    }}>admin 가입 요청
                    </button>
