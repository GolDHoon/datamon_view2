...
const Step2 : React.FC<{
    ...
    setSignUpData : any;
          ...
          nextStep,prevStep,handlePhoneVerifyClick,isPhoneVerified,signUpData,setSignUpData
      }) => {
    ...
    const handleRequestVerification = () => {
        restApi('get','/member/vrfn/sms/req',{
    ...
    const duplicateCheck = (e : React.MouseEvent<HTMLButtonElement>) => {
        restApi('post','/member/checkIdDuplicate',{
            userId: signUpData.username,companyId: getSession("companyIdx"),
        }).then(response => {
            // @ts-ignore
    ...
    const handleCheckVerify = () => {
        restApi('post','/member/vrfn/sms/check',{
    ...
    return (<div className="fade">
        <div className="content">
            ...
            <div className="input_box">
                ...
                {showVerification && !isPhoneVerified && ( // 인증번호 입력 필드를 조건부로 렌더링
                    <div>
                        <input
                            type="number"
                            placeholder="인증번호를 입력하세요"
                            value={vrfnCode}
                            onChange={(event) => setVrfnCode(event.target.value)}
                        />
...
const Step3 : React.FC<{
    ...
    const handleRequestVerification = () => {
        restApi('get','/member/vrfn/mail/req',{
    ...
    const handleCheckVerify = () => {
        restApi('post','/member/vrfn/mail/check',{
            idx: vrfnId.current,verificationCode: vrfnCode,
        }).then(response => {
            if (response.status === 200) {
                alert(response.data.message);
...
const SignUp : React.FC = () => {
    ...
    const [signUpData,setSignUpData] = useState({
        job: '',name: '',reason: '',username: '',password: '',phone: '010',email: ''
    });
    // 전화번호 인증 상태
