...
const Column : React.FC<ColumnProps> = ({column,index,moveColumn,columnWidths,onMouseDown,onSort,sortOrder}) => {
    ...
    const [{isDragging},drag] = useDrag({
        ...
        item: {type: ItemType,index},collect: (monitor) => ({
    ...
    return (<div
            ...
            style={{
                ...
                display: 'flex',alignItems: 'center',position: 'relative',opacity: isDragging ? 0.35 : 1, // 드래그 중일 때 불투명도 변경
...
const CommonDataGrid : NextPage<DataGridProps> = ({
    ...
    const [tabFilterList,setTabFilterList] = useState<any[]>(columns.map((column : any) => {
        return {
            key: column.key,name: column.name,type: column.filterType
        }
    }));
    ...
    const handleDownloadCSV = (event : any) => {
        ...
        // 현재 표시된 행 가져오기
        const data = currentRows.map(row => currentColumns.map(column => String(row[column.key])).join(',')).join('\n');
    ...
    const handleSort = (columnKey : string) => {
        ...
        if (existingSort) {
            if (existingSort.direction === 'asc') {
                newSortConfig = sortConfig.map(config => config.key === columnKey ? {
                    key: columnKey,
                    direction: 'desc'
                } : config);
    ...
    const sortedRows = useCallback((currentRows : any[]) => {
    ...
    const handleTabAndFilterClick = (e : React.MouseEvent<HTMLLIElement,MouseEvent>,data : any) => {
        ...
        if (clickedElement.classList.contains('text')) {
            ...
            setAutoCompleteFilterList([...new Set(rows.map(row => JSON.stringify({
                key: [data.key],[data.key]: row[data.key]
    ...
    const handleTextFilter = (event : any) => {
        if (event.target.value === '') {
            ...
            // @ts-ignore
            setAutoCompleteFilterList(autoTempCompleteFilterList.filter(item => item[item.key].includes(event.target.value)));
        }
        return null;
    }
    ...
    const handleCheckboxAll = () => {
        ...
        if (newAllCheckSelected) {
            ...
            const newFilters = columns.map(data => ({
                key: data.key,name: data.name,type: data.filterType,
            }));
            ...
            setSelectedColumns([]);
            setTabFilterList(prev => {
                const updatedFilterList = prev.filter(filter => !columns.some(data => data.name === filter.name));
                return updatedFilterList;
            });
        }
    };
    ...
    const handleCheckboxChange = (data : any) => {
        if (selectedColumns.includes(data.name)) {
            ...
            setTabFilterList(prev => prev.filter(filter => filter.name !== data.name));
        } else {
    ...
    return (<DndProvider backend={HTML5Backend}>
                ...
                <div className="top_section">
                    <div className="filter_wrap">
                        <div
                                ...
                                <div className="output">
                                    ...
                                    <div className={`filter_box ${filterClass}`}>
                                        ...
                                        <div className="check_type">
                                            <ul>
                                                {checkList.map((check,index) => (<li key={index}>
                                        ...
                                        <div className={`text_type ${autoCompleteFilterList.length === 0 ? '' : ''}`}>
                                            ...
                                            <ul>
                                                {/* 검색했을 때 뜨는 자동 결과값 */}
                                                {autoCompleteKeyword !== '' && (autoCompleteFilterList.map((auto,index) => (
                                                        <li key={index}
                                                            onClick={(event) => handleAutoCompleteFilterRegister(auto[auto.key])}>
                                                            <IoIosSearch color="ccc" size='14'/>
                                                            <span> {auto[auto.key]}</span> <GoArrowUpLeft color="ccc"
                                                                                                          size='14'/>
                                                        </li>)))}
                                            </ul>
                                        </div>
                            ...
                            {!!useTabFilterButton ? (useTabFilterButton ? (<div className='tab'>
                                        ...
                                        <div className="output_t">
                                            <ul className="list">
                                                ...
                                                {columns.map((data,index) => (<li key={index}>
                                                        <input
                                                            type="checkbox"
                                                            id={data.name}
                        ...
                        <div className="right">
                            {!!useExcelDownload ? (useExcelDownload ? (
                                    <button type="button" className="excel"><PiMicrosoftExcelLogoFill
                                        onClick={(event) => handleDownloadCSV(event)} color="#fff"/>
                                    </button>) : null) : null}
                        </div>
                    </div>
            ...
            <div ref={tableRef} className='table_content'>
                <div className='table_head'>
                    {currentColumns.map((column,index) => (<Column
                            key={index}
                            column={column}
                            index={index}
            ...
            {!!useNewContentButton ? (useNewContentButton ? (<div className='button_box'>
