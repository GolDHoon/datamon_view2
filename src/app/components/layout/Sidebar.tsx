...
export default function Sidebar () {
    ...
    return (<nav className={isActive1 ? 'min' : ''}>
        <h1><Link href="/home">DATAMON</Link></h1>
        <i className="right_toggle" onClick={() => setIsActive1( !isActive1)}><BsArrowLeftShort color="fff"
                                                                                                size="26"/>arrow</i>
        <div className="nav_wrap">
            <ul className="gnb">
                ...
                {["USTY_INME","USTY_CLME","USTY_CRAC","USTY_CAME"].includes(userType) ? null : (
                    <li className={`mli ${isActive3 ? 'on' : ''}`} onClick={() => setIsActive3( !isActive3)}>
                        <PiUserListBold
                        ...
                        <ul>
                            ...
                            {["USTY_MAST","USTY_DEVL"].includes(userType) ? (
                                <li><Link href={"/admin/list"}>admin 계정 목록</Link></li>) : null}
                            <li><Link href={"/user/list"}>사용자 계정 목록</Link></li>
                            {/*<li>고객DB 매핑 요청 승인</li>*/}
                        </ul>
                    </li>)}
                <li className={`mli ${isActive4 ? 'on' : ''}`} onClick={() => setIsActive4( !isActive4)}>
                    <FaDatabase size="20"/> 고객DB 관리 <IoIosArrowDown/>
                    <ul>
                ...
                <li className={`mli ${isActive2 ? 'on' : ''}`} onClick={() => setIsActive2( !isActive2)}>
                    <IoStatsChart size="20"/> 실적 통계 <IoIosArrowDown/>
                    <ul>
