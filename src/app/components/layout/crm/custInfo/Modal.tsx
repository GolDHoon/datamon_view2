...
export default function Modal ({isOpen, onClose, typeList, dataJson} : ModalProps) {
    const [currentData, setCurrentData] = useState<any>({
        custInfoIdx: dataJson?.idx, dataList: []
    });
        ...
        if (typeList.length > 0 && dataJson) {
            const newDataList = typeList.map((item : any) => {
                ...
                if (item?.key === "cdbsCode") {
                    return {
                        key: item?.key, value: GetConst("cdbsCode").find((data : any) => {
                            return data.value === value
                        }).key, columnType: item.columnType, filterType: item.filterType, name: item.name
                    };
                }
                return {
                    key: item?.key,
            ...
            setCurrentData({
                custInfoIdx: dataJson?.idx, dataList: newDataList
            });
        }
    }, [typeList, dataJson]);
                ...
                <ul>
                    {currentData.dataList.map((item : any, index : number) => {
                        ...
                        switch (item.filterType) {
                            case "text":
                                inputComponent = (<li key={index}>
                                    <label>{item.name}</label>
                                    {item.key === "memo" ? (<textarea
                                        value={item.value}
